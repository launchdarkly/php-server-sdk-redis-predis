...
class RedisFeatureRequesterTest extends TestCase
{
    ...
    {
        $client = self::client($clientPrefix);
    ...
    private function setupForPrefix(Client $client, ?string $prefix, string $flagKey, string $segmentKey, int $flagVersion): void
    {
    ...
    private function clearExistingData(Client $client): void
    {
        $client->flushall();
    }

    private function makeRequester(Client $client, ?string $prefix): FeatureRequester
    {
        $factory = Redis::featureRequester($client, ['prefix' => $prefix]);
        return $factory('', '', []);
    }
    ...
    private static function realPrefix(?string $prefix): string
    {
        if ($prefix === null || $prefix === '') {
            return Redis::DEFAULT_PREFIX;
        }

        return $prefix;
    }

    private static function client(?string $prefix): Client
    {
        if ($prefix === null) {
            return new Client();
        }

        return new Client([], ['prefix' => $prefix]);
    }
}
